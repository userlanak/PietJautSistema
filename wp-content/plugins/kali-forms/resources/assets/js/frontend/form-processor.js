...
export default class FormProcessor {
	...
	handleFileUploads() {
		...
		[...fields].map( field => {
			...
			if ( field.hasAttribute( 'multiple' ) ) {
				generalSettings.labelIdle = sprintf( __( 'Drag & Drop your files (max %s) or', 'kaliforms' ), options.maxFiles ) + ' <span class="filepond--label-action"> ' +
						__( 'Browse', 'kaliforms' ) + ' </span>';
			}
	...
	handleRecaptcha() {
		...
		let grecaptchas = this.form.querySelectorAll( '[data-field-type=\'grecaptcha\']' );
	...
	verifyRecaptchaCallback( res ) {
		...
		this.axios.post( KaliFormsObject.ajaxurl, this.Qs.stringify( data ) ).then( r => {
			if ( this.submitButton !== null ) {
	...
	scrollTyIntoView() {
		document.addEventListener( 'kaliformShownThankYouMessage', evt => {
			this.scrollThankYouIntoView
					? document.querySelector( `#kaliforms-thank-you-message-${this.formId}` ).scrollIntoView( {
						behavior: 'smooth',
						block: 'start'
					} )
					: false;
	...
	async _processForm( formData ) {
		...
		return axios.post( KaliFormsObject.ajaxurl, this.Qs.stringify( data ) ).then( r => {
			if ( r.data.hasOwnProperty( 'error' ) ) {
				this.throwError();
			} else {
				...
				if ( r.data.hasOwnProperty( 'redirect_url' ) && r.data.redirect_url !== '' ) {
					setTimeout( () => window.location.href = r.data.redirect_url, parseInt(r.data.redirect_timeout) * 1000 );
				}
			}
		} ).catch( e => {
			console.log( e );
	...
	_resetFormAndShowThankYouMessage( response ) {
		this.form.insertAdjacentHTML( 'beforebegin', `<div id="kaliforms-thank-you-message-${this.formId}">${response.thank_you_message}</div>` );
		this.form_data = response.form_data;
		document.dispatchEvent( new CustomEvent( 'kaliformShownThankYouMessage', { detail: this } ) );
		this.resetForm();
		setTimeout( () => document.getElementById( `kaliforms-thank-you-message-${this.formId}` ).remove(), 5000 );
	}
